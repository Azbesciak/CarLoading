...
public class SolutionTest {
    ...
    public void maxDistanceTest() {
        ...
        assertEquals(20, solution.getPurpose().getMaxDistance());
        ...
        assertEquals(21, solution1.getPurpose().getMaxDistance());
        ...
        assertEquals(27, solution2.getPurpose().getMaxDistance());
        ...
        assertEquals(27, solution3.getPurpose().getMaxDistance());
    ...
    public void occupiedFieldTest() {
        ...
        assertEquals(50, new Solution(placements).getPurpose().getOccupiedPlace());
        final PackagePlacement p2 = preparePlacement(5, 10, 10, 10);
        placements.add(p2);
        assertEquals(50, new Solution(placements).getPurpose().getOccupiedPlace());
        final PackagePlacement p3 = preparePlacement(10, 10, 0, 0);
        placements.add(p3);
        assertEquals(150, new Solution(placements).getPurpose().getOccupiedPlace());
        final PackagePlacement p4 = preparePlacement(10, 10, 10, 0);
        placements.add(p4);
        assertEquals(250, new Solution(placements).getPurpose().getOccupiedPlace());
    }

    private PackagePlacement preparePlacement(int width, int length, int x, int y) {
        return PackagePlacement.builder()
                .pack(Package.builder().host(Host.builder().width(width).length(length).build())
                        .canOtherPackageBePlacedOn(true).canBePlacedOnPackage(true).build()).placement(
                new Placement(x, y)).build();
    }
}
